...
% Mencari OCV vs SOC
data = readtable(fullfile(fileparts(mfilename('fullpath')), 'Database', 'OC_Current_Voltage.csv'));
Q_nominal = 1.1;
time = data.Step_Time;
...
for i = 2:length(current_cha)
    soc_cha(i) = soc_cha(i-1) + (current_cha(i) * dt_cha(i) / (Q_nominal * 3600)) * 100; % Step time dalam satuan detik sehingga ditambahkan 3600
end
